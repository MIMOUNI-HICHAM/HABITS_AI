...
def get_sleep_data():
    try:
        with get_connection() as conn:
            ...
            if not current_user:
                return {
                    ...
                    'streaks': {'current': 0, 'longest': 0, 'minimumHours': 6}
                }
        ...
        print(f"Error in get_sleep_data: {str(e)}")
...
def add_sleep_session(data):
    with get_connection() as conn:
        ...
        c.execute('''
            ...
            VALUES (%s, %s, %s, %s, %s)
        ''', (current_user, data['date'], data['hours'], data['quality'], data.get('notes', '')))
        conn.commit()
        return {'message': 'Sleep session added successfully'}

def get_today_sessions():
    today = datetime.now().strftime('%Y-%m-%d')
    with get_connection() as conn:
        c = conn.cursor(dictionary=True)
        c.execute('SELECT * FROM sleeping_entries WHERE date = %s', (today,))
        rows = c.fetchall()
        return {
            'today': today,
            'sessions': [{
...
def get_calendar_data():
    with get_connection() as conn:
        ...
        c.execute('''
            SELECT DATE(date) as date, 
                   ...
                   GROUP_CONCAT(DISTINCT quality SEPARATOR ',') as qualities
            FROM sleeping_entries
            WHERE date BETWEEN %s AND %s AND user_id = %s
            GROUP BY DATE(date)
            ORDER BY date
        ...
        for row in c.fetchall():
            ...
            qualities = row['qualities'].split(',') if row['qualities'] else []
            daily_data[date_str] = {
                'hours': float(row['total_hours']) if row['total_hours'] else 0,
                'qualities': qualities
            }
    ...
    while current_date <= end_date:
        ...
        if current_date.year == 2025:
            day_data = daily_data.get(date_str, {'hours': 0, 'qualities': []})
...
def init_db():
    """Initialize the sleeping_entries table using MySQL syntax."""
    try:
        with get_connection() as conn:
            c = conn.cursor()
            c.execute('''
            ...
            ''')
            conn.commit()
            print("Sleeping entries table initialized successfully")
    except Error as e:
        print(f"Error initializing sleeping_entries table: {e}")
